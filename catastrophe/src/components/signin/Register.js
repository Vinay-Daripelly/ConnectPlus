    ...
    try {
      ...
      for (const key in formData) {
        if (key === 'profilePhoto') {
          ...
          // Append the field if the user type is alumni, or if the field is not one of the alumni-specific fields
          data.append(key, formData[key]);
        }
      }
  
      // Conditionally set the URL based on user type
      ...
      // Conditionally set the URL for fetching user data
      ...
      // After user creation, retrieve the user data including the image URL
      const userDataResponse = await axios.get(fetchUrl);
  
      // Assuming the server returns the image URL in the response data
      const imageUrlFromServer = userDataResponse.data.profilePhoto;
      console.log("Image URL from server:", imageUrlFromServer);
